...
async def fetch_detail(context, job):
    """
    通过信号量控制，并行抓取详情页
    """
    async with sem:
...
async def main():
    async with async_playwright() as p:
        ...
        print(f"[+] 发现 {len(jobs_to_crawl)} 个唯一岗位，准备并行采集...")

        # 5. 并发执行详情页抓取
        tasks = [fetch_detail(context, job) for job in jobs_to_crawl]
